...
C = 4186#4.2e6            # Heat capacity of water
...
def snowball_earth(nbins=18, dt=1., tstop=10000, lam=100., spherecorr=True,
    ...
    for i in range(nstep):
        ...
        if insol:
            ...
            ins = (dt_sec/(rho*C*mxdlyr)) * (insolation(lats)*(1-0.3)-epsilon*sigma*(Temp+273.15)**4)
            Temp += ins
            if debug:
                print(f'ins term: {ins}')
                print(f'Temp after insol: {Temp}')
...
def test_snowball(tstop=10000):
    ...
    lats, t_diff = snowball_earth(tstop=tstop, spherecorr=False, insol=False)

    # Get diffusion + spherical correction:
    lats, t_sphe = snowball_earth(tstop=tstop, insol=False)
    print(t_sphe[0])
...
#lats, t_ins =  snowball_earth(tstop=3)

test_snowball()

# dlat, lats = gen_grid()
# print(insolation(lats))
# print(temp_warm(lats))

# snowball_earth(tstop=3, debug=True)